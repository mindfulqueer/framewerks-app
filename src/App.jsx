    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.95)", zIndex: 1500, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ position: "relative", width: 340, height: 340, marginBottom: 28 }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: accentColor, boxShadow: `0 0 ${20 + breathY * 25}px ${accentColor}${Math.round(30 + breathY * 50).toString(16).padStart(2, '0')}`, transform: `translateY(${yOffset}px)`, transition: "transform 0.15s ease-out, box-shadow 0.3s ease" }} />
        </div>
...
export default function FramewerksApp() {
  ...
  return (<div style={{ fontFamily: "'Bebas Neue',sans-serif", background: "#0A0A0A", minHeight: "100vh", color: "#F0EDE8", maxWidth: 430, margin: "0 auto" }}><style>{CSS}</style>
    ...
    {view === "home" && (<div style={{ paddingBottom: 90 }}>
      ...
      <div style={{ padding: "0 24px 18px" }}><p style={{ ...S.label, marginBottom: 10 }}>Your Program</p>{program ? (<>{<div style={{ background: "#111", borderRadius: 16, border: "1px solid #1E1E1E", overflow: "hidden", marginBottom: 12 }}><div style={{ height: 4, background: ACCENT }} /><div style={{ padding: "16px" }}><p style={{ fontSize: 28, letterSpacing: 1.5 }}>{program.name}</p>{program.tags?.length > 0 && <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>{program.tags.map(t => <span key={t} style={{ ...S.body, fontSize: 10, fontWeight: 700, background: ACCENT + "18", color: ACCENT, borderRadius: 5, padding: "2px 8px" }}>{t}</span>)}</div>}<p style={{ ...S.body, fontSize: 12, color: "#555", marginTop: 6, lineHeight: 1.5 }}>{program.description}</p></div></div>}{program.phases.map((phase, pi) => (<div key={phase.id} style={{ marginBottom: 16 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><p style={{ fontSize: 20, letterSpacing: 1 }}>{phase.name}</p><p style={{ ...S.body, fontSize: 11, color: "#555" }}>{phase.weeks} weeks</p></div>{phase.days.map((day, di) => { const totalEx = (day.warmup?.length || 0) + (day.exercises?.length || 0) + (day.cooldown?.length || 0); return (<div key={day.id} className="su pr" style={{ animationDelay: `${di * 0.06}s`, background: "#111", borderRadius: 14, border: "1px solid #1E1E1E", overflow: "hidden", marginBottom: 8, cursor: "pointer" }} onClick={() => startWorkout(phase, day)}><div style={{ padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><p style={{ fontSize: 20, letterSpacing: 1 }}>{day.label}</p><p style={{ ...S.body, fontSize: 11, color: "#555", marginTop: 2 }}>{totalEx} exercises</p></div><div style={{ background: ACCENT, borderRadius: 10, padding: "9px 18px" }}><span style={{ ...S.body, fontSize: 13, fontWeight: 700, color: "#fff" }}>Start</span></div></div></div>); })}</div>))}</>) : (<div style={{ background: "#111", borderRadius: 14, padding: "24px 16px", border: "1px dashed #2A2A2A", textAlign: "center" }}><p style={{ fontSize: 28 }}>{"\u26A1"}</p><p style={{ ...S.body, fontSize: 13, color: "#555", marginTop: 8 }}>No program assigned yet. Your coach will send one to you.</p></div>)}</div>
