        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px",marginBottom:12}}>
          ...
          {setData[ex.id]?.map((set,si)=>(
            <div key={si} style={{display:"grid",gridTemplateColumns:"28px 1fr 1fr 40px",gap:8,alignItems:"center",padding:"8px 0",borderTop:`1px solid ${C.border}`,opacity:set.done?0.6:1}}>
              ...
              <input type="number" inputMode="decimal" placeholder={ex.startWeight||"0"} value={set.weight}
...
function DailyCheckin({ today, todayWB, saveWB, user, showHistory=false, wellbeing={} }) {
  ...
  if(submitted && allAnswered){
    return (
      <div style={{background:C.card,border:`1px solid ${C.accentGreen}44`,borderRadius:12,padding:"16px",marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div>
            <div style={{fontSize:18,fontFamily:F.display,color:C.accentGreen}}>✓ CHECK-IN COMPLETE</div>
            <div style={{fontSize:11,fontFamily:F.body,color:C.textMuted,marginTop:2}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {CHECKIN_METRICS.map(m=>(
            <div key={m.key} style={{background:C.surface,borderRadius:10,padding:"10px 12px",display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:20}}>{m.icon}</span>
              <div>
                <div style={{fontSize:10,fontFamily:F.body,color:C.textMuted,fontWeight:700,letterSpacing:"0.1em"}}>{m.label}</div>
        ...
        <div style={{fontSize:10,fontFamily:F.body,color:C.textDim,textAlign:"center",marginTop:10}}>
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"20px",marginBottom:12}}>
      {/* Progress bar */}
      <div style={{height:3,background:C.surface,borderRadius:2,marginBottom:20,overflow:"hidden"}}>
        <div style={{height:"100%",background:C.accent,width:`${progress}%`,transition:"width 0.3s ease"}} />
      </div>

      <div style={{fontSize:10,fontFamily:F.body,color:C.textMuted,fontWeight:700,letterSpacing:"0.15em",marginBottom:4}}>
      ...
      <div style={{fontSize:14,fontFamily:F.body,color:C.textMuted,marginBottom:20}}>{metric.question}</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {[1,2,3,4,5].map(n=>(
          <button key={n} onClick={()=>handleAnswer(metric.key,n)}
            ...
            <span style={{fontFamily:F.display,fontSize:18,color:todayWB[metric.key]===n?C.accent:C.textMuted}}>{n}</span>
      ...
      {step > 0 && (
        <button onClick={()=>setStep(s=>s-1)}
          style={{marginTop:12,background:"transparent",border:"none",color:C.textMuted,cursor:"pointer",fontFamily:F.body,fontSize:12}}>
          ← Back
        </button>
      )}
    </div>
